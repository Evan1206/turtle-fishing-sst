...
6. **「如何判讀」說明區塊（2026-07-27 新增）**：獨立的可展開區塊或彈出視窗
   （不是塞進既有的免責文字裡，是額外一塊，可以配合小龜吉祥物用白話講），
   至少要涵蓋以下四點，逐條寫清楚，不要籠統帶過：
   - 每個網格代表什麼：0.25° × 1 週的一格，顏色深淺是**那一格那一週**的
     `fishing_hours`（漁船作業小時），不是船的數量、也不是累計整個研究期間的量。
   - 海龜圖示大小代表什麼：**定位次數**（同一格同一週的 GPS/Argos 定位點數），
     不是海龜的數量、也不是海龜的體型或年齡。
   - Spearman ρ 怎麼讀：這是**排序相關**的方向與強度，不是「重疊了多少 %」，
     ρ 接近 0 代表關聯很弱即使統計顯著（呼應結果面板已有的效應量提示）。
   - 時間滑桿在做什麼：**切換靜態的某一週**，不是動畫播放、不會自動前進，
     每一格都是各自獨立算出來的歷史快照。
   這個區塊不需要新的資料或後端邏輯，純粹是 `web/` 前端加內容，
   不影響 `export_web_data.py` 或任何 Python 端模組。
...
- **不接受**：預設瀏覽器樣式（無 CSS）、把熱區資料色階換成粉彩/彩虹漸層導致
  ...
  時會對照這份清單。
- **手機版兩項微調（2026-07-27，構思層實測窄螢幕發現，非阻塞但排進待辦）**：
  1. 圖例目前浮在地圖右下角，在窄螢幕（約 ≤480px 寬）下會蓋住地圖寬度約四成，
     比桌面版明顯。窄螢幕下改成可收合（例如一個小按鈕展開/收起，預設收起或
     縮小尺寸），桌面版不受影響。
  2. 圖層切換按鈕（漁船作業/海龜定位）目前點擊區域高度約 38px，低於一般建議
     的 44px 觸控目標下限。調整 padding 讓點擊區域達到至少 44px 高，不用改
     視覺呈現的大小，只要點擊/觸控的有效範圍夠大即可。
...
- [人審] 「如何判讀」說明區塊存在，且網格意義、海龜圖示大小意義、Spearman ρ
  怎麼讀、時間滑桿在做什麼這四點都有寫清楚，不是籠統帶過。
